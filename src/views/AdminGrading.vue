...
const manualAnswers = computed(() => {
  return (detail.value?.answers || []).filter((answer: any) => isManualQuestion(answer));
});
...
const submitGrades = async () => {
  ...
  try {
    ...
    alert('批改已保存，成绩已同步到考生考试信息。');
  } catch (err: any) {
    alert(err.message || '保存批改失败');
  } finally {
    saving.value = false;
  }
};
...
<template>
  <div class="max-w-7xl mx-auto px-4 py-12">
      ...
      <main class="flex-1 min-w-0">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div>
            ...
            <h1 class="font-display text-3xl text-black-700">考卷批改</h1>
          ...
          <table class="w-full min-w-[980px] text-left">
            <thead class="bg-black-50 border-b border-black-100">
              <tr>
                ...
                <th class="px-6 py-5 text-xs font-bold text-black-500 uppercase tracking-widest">参与考试</th>
            ...
            <tbody class="divide-y divide-black-50">
              ...
              <template v-else>
                <tr v-for="row in submissions" :key="row.id" class="hover:bg-gold-50/30 transition-colors">
                  ...
                  <td class="px-6 py-5 font-mono font-bold text-gold-600">
                    {{ row.score ?? '待批改' }}
                  </td>
    ...
    <div v-if="selectedSubmission" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 p-4 overflow-y-auto">
      <div class="bg-white max-w-5xl mx-auto my-8 shadow-2xl">
        <div class="sticky top-0 bg-white border-b border-black-100 p-6 flex items-start justify-between z-10">
          <div>
        ...
        <div class="sticky bottom-0 bg-white border-t border-black-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div class="text-sm text-black-500">
            <span v-if="manualAnswers.length === 0">该试卷全部为客观题，系统已完成评分。</span>
